...
def decrypt(encrypted_msg, key):
    ...
    for i in range(msg_len):
        ...
        # Change back to chr type and add to text
        decrypted_msg += chr(integer).lower()
        # Repeat for the second column
...
def make_key(text):
    ...
    '''while True:
        ...
        key = create_matrix_of_integers_from_string(cipher)
        determinant = key[0][0] * key[1][1] - key[0][1] * key[1][0]
        determinant = determinant % 26
        ...
        elif np.amax(key) > 26 and np.amin(key) < 0:
            print("Only a-z characters are accepted")
            print(np.amax(key), np.amin(key))
        else:
            break'''
    key = create_matrix_of_integers_from_string(text)
    determinant = key[0][0] * key[1][1] - key[0][1] * key[1][0]
    determinant = determinant % 26
    inverse_element = find_multiplicative_inverse(determinant)
